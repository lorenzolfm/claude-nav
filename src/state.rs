//! The vocabulary: what a status means, what a row is called, how rows sort,
//! and how long an agent has been where it is.
//!
//! Each surface renders this differently and none of them decides it. The table
//! below is luneta's, and a surface that disagrees with it is a second mapping.
...
impl Status {
    ...
    /// The glyph for a status that does not turn.
    ///
    /// `None` is `busy`, and the caller draws its own spinner. Which status
    /// turns is decided here; how many dots turn is not. The tray's menu greys
    /// three dots into specks, so it spends seven, and luneta spends three.
    pub fn fixed_glyph(&self) -> Option<&'static str> {
        match self.face {
            Face::Fixed(glyph) => Some(glyph),
            Face::Spinner => None,
        }
    }
...
impl Entry {
    /// The age to show: what the snapshot measured, plus the time since it was
    /// taken. Without the offset an agent that has waited three minutes shows
    /// the same number for as long as you look at it, in the one column that
    /// reports whether an agent is stuck.
    ///
    /// The same offset reaches every row, so it cannot change a comparison.
    pub fn age(&self, since: u64) -> u64 {
        self.age_s.saturating_add(since)
    }
}
...
impl Badge {
    /// How many agents wait for you. `Blind` is not zero: it is no answer.
    pub fn waiting(self) -> Option<NonZeroUsize> {
        match self {
            Badge::Waiting(n) => Some(n),
            Badge::Quiet | Badge::Blind => None,
        }
    }
...
/// `<1m`, `47m`, `3h`, `2d`. Shared, so that an age reads the same in the tray
/// menu and in the launcher.
...
mod tests {
    ...
    fn the_age_counts_on_after_the_snapshot_was_taken() {
        ...
        assert_eq!(humanise(snap.entries[0].age(0)), "<1m");
        assert_eq!(snap.since(NOW + 90), 90);
        assert_eq!(humanise(snap.entries[0].age(90)), "2m");
    }
    ...
    #[test]
    fn an_unknown_status_keeps_the_word_it_was_given() {
        let snap = snapshot(&[row("compacting", 120)], NOW);
        let status = &snap.entries[0].status;
        assert_eq!(status.as_str(), "compacting");
        assert_eq!(status.fixed_glyph(), Some(UNKNOWN_GLYPH));
    }
    ...
    fn a_shell_has_its_own_picture_and_no_rank_of_its_own() {
        ...
        assert_eq!(entry.status.fixed_glyph(), Some("🐚"));
        assert!(!entry.status.is_spinning(), "a shell does not turn");
    }
    ...
    #[test]
    fn busy_is_the_one_status_with_no_fixed_glyph() {
        let busy = snapshot(&[row("busy", 5)], NOW);
        assert!(busy.any_spinning());
        assert_eq!(busy.entries[0].status.fixed_glyph(), None);

        for still in ["waiting", "idle", "shell", "compacting"] {
            let snap = snapshot(&[row(still, 5)], NOW);
            assert!(!snap.any_spinning(), "{still}");
            assert!(snap.entries[0].status.fixed_glyph().is_some(), "{still}");
        }
    }

    #[test]
    fn only_a_waiting_badge_carries_a_number() {
        assert_eq!(snapshot(&[row("idle", 5)], NOW).badge().waiting(), None);
        assert_eq!(
            snapshot(&[row("waiting", 5)], NOW).badge().waiting(),
            Some(waiting(1))
        );
        assert_eq!(Badge::Blind.waiting(), None, "blind is no answer, not zero");
    }
